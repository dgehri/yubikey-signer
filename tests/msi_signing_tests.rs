...
//!
//! Remote-signing tests are skipped automatically unless environment
...
fn get_remote_config() -> Option<(String, String, Option<String>, Option<String>)> {
    ...
    Some((url, token, cf_client_id, cf_client_secret))
}
...
async fn test_msi_remote_signing() {
    ...
    let Some((proxy_url, proxy_token, cf_client_id, cf_client_secret)) = get_remote_config() else {
        eprintln!(
    ...
    let mut config = RemoteSignerConfig::new(&proxy_url, &proxy_token);
    if let Some(id) = cf_client_id {
        if let Some((name, value)) = id.split_once(':') {
            config = config.with_header(name.trim(), value.trim());
        }
    }
    if let Some(secret) = cf_client_secret {
