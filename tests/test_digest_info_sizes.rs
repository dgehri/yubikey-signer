...
use yubikey_signer::{HashAlgorithm, authenticode::create_digest_info};
...
fn test_digest_info_sizes() {
    ...
    println!("SHA-256 DigestInfo size: {} bytes", sha256_digest_info.len());
    assert_eq!(sha256_digest_info.len(), 51); // Should be 19 (header) + 32 (hash)
    
    // Test SHA-384
    let sha384_hash = vec![0u8; 48]; 
    let sha384_digest_info = create_digest_info(&sha384_hash, HashAlgorithm::Sha384).unwrap();
    println!("SHA-384 DigestInfo size: {} bytes", sha384_digest_info.len());
    assert_eq!(sha384_digest_info.len(), 67); // Should be 19 (header) + 48 (hash)
    
    // Test SHA-512
    let sha512_hash = vec![0u8; 64];
    let sha512_digest_info = create_digest_info(&sha512_hash, HashAlgorithm::Sha512).unwrap();
    println!("SHA-512 DigestInfo size: {} bytes", sha512_digest_info.len());
    assert_eq!(sha512_digest_info.len(), 83); // Should be 19 (header) + 64 (hash)
