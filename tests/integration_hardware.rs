...
async fn test_real_yubikey_connection_and_certificate_retrieval() {
    ...
    let config = SigningConfig {
        pin,
        piv_slot: 0x9a, // Use Authentication slot where we found the certificate
        hash_algorithm: HashAlgorithm::Sha256, // Matches -h sha256
        timestamp_url: Some("http://ts.ssl.com".to_string()), // Matches -ts
...
async fn test_yubikey_certificate_matches_file() {
    ...
    ops.authenticate(&pin).expect("YubiKey authentication failed");
    
    // Try to get certificate from slot 0x9a (authentication slot where we know there's a cert)
    let result = ops.get_certificate(0x9a);
    
    match result {
        Ok(cert) => {
            println!("SUCCESS: Retrieved certificate from YubiKey slot 0x9c");
        ...
        Err(e) => {
            // No certificate in 0x9c, try other slots
            println!("No certificate in slot 0x9c: {}", e);
            
            // Try other common slots
            let slots_to_try = [0x9a, 0x9d, 0x9e]; // Authentication, Key Management, Card Authentication
            let mut found_cert = false;
            
            for slot in slots_to_try {
                match ops.get_certificate(slot) {
                    Ok(cert) => {
                        println!("SUCCESS: Retrieved certificate from YubiKey slot 0x{:02x}", slot);
                        println!("Certificate subject: {:?}", cert.tbs_certificate.subject);
                        assert!(!cert.tbs_certificate.serial_number.as_bytes().is_empty());
                        found_cert = true;
                        break;
...
fn test_yubikey_basic_connection() {
    ...
    match result {
        ...
        Err(e) => {
            panic!("Failed to connect to YubiKey: {}. Is YubiKey plugged in?", e);
        }
    }
}
