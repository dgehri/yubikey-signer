...
mod certificate_tests {
    ...
    fn test_certificate_validity_periods() {
        ...
        for slot in test_slots {
            println!("Testing certificate in slot 0x{:02x}", slot);
    ...
    fn test_signing_with_various_hashes(ops: &mut YubiKeyOperations, slot: u8) {
        ...
        for (name, hash) in test_hashes {
            match ops.sign_hash(&hash, slot) {
                Ok(signature) => {
                    println!("    ✅ {} hash: {} bytes -> {} byte signature", name, hash.len(), signature.len());
                }
                Err(e) => {
...
mod algorithm_tests {
    ...
    fn test_algorithm_fallback_behavior() {
        ...
        match ops.sign_hash(&test_data, 0x9a) {
            Ok(signature) => {
                println!("✅ Fallback chain succeeded: {} byte signature", signature.len());
        ...
        for hash_alg in hash_algorithms {
            for slot in &slots_to_test {
                ...
                let result = tokio_test::block_on(
                    sign_pe_file(temp_file_obj.path(), &output_path, config)
                );
...
mod signature_format_tests {
    ...
    fn test_signature_consistency() {
        ...
        for i in 0..iterations {
            match ops.sign_hash(&test_hash, 0x9a) {
                Ok(signature) => {
        ...
        if signatures.len() >= 2 {
            ...
            println!("✅ All signatures have consistent length: {} bytes", first_len);
    ...
    fn test_boundary_hash_sizes() {
        ...
        for (name, hash) in boundary_cases {
            match ops.sign_hash(&hash, 0x9a) {
                Ok(signature) => {
                    println!("✅ {}: {} bytes -> {} byte signature", name, hash.len(), signature.len());
                }
                Err(e) => {
...
mod error_handling_tests {
    ...
    fn test_error_propagation() {
        ...
        let result = tokio_test::block_on(
            sign_pe_file(temp_file.path(), &output_path, invalid_config)
        );
    ...
    fn test_concurrent_yubikey_access() {
        ...
        let handles: Vec<_> = (0..5).map(|i| {
            thread::spawn(move || {
                ...
                match result {
                    Ok(_ops) => {
                        println!("Thread {} connected successfully", i);
                        true
                    }
                    Err(e) => {
        ...
        println!("Concurrent access: {}/{} threads succeeded", success_count, results.len());
        
        // This is hardware dependent, but shouldn't crash
        assert!(success_count <= 1, "At most one thread should connect to YubiKey");
    }
    ...
    fn test_memory_safety_with_large_inputs() {
        ...
        let large_config = SigningConfig {
            pin: "A".repeat(1000), // Very long PIN
            piv_slot: 0x9a,
            hash_algorithm: HashAlgorithm::Sha256,
            timestamp_url: Some("http://".to_string() + &"a".repeat(1000) + ".com"), // Very long URL
            embed_certificate: true,
        };
        ...
        let result = tokio_test::block_on(
            sign_pe_file(temp_file.path(), &output_path, large_config)
        );
...
mod real_world_tests {
    ...
    fn test_realistic_signing_workflow() {
        ...
        let result = tokio_test::block_on(
            sign_pe_file(temp_file.path(), &output_path, config)
        );
        ...
        match result {
            Ok(_) => {
                ...
                if let Ok(metadata) = std::fs::metadata(&output_path) {
                    ...
                    assert!(metadata.len() > realistic_pe.len() as u64, "Signed file should be larger");
                }
