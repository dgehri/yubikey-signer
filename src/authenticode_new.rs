...
mod tests {
    ...
    fn create_test_certificate() -> Certificate {
        ...
        // In a real test, you would load an actual certificate
        let test_cert_der = vec![
            0x30, 0x82, 0x01, 0x23, // Basic certificate structure
            // ... more certificate data would go here
        ];
        
        // For testing, create a dummy certificate or use a known test certificate
        // Since Certificate::from_der needs the Decode trait, we'll create a placeholder
        // This would normally parse a real certificate
        use std::str::FromStr;
        let cert_pem = "-----BEGIN CERTIFICATE-----\nMIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA3VoPN9PKUjKFLMwOge9+\n-----END CERTIFICATE-----";
        Certificate::from_str(cert_pem).unwrap_or_else(|_| {
            // Create a minimal certificate structure for testing
            panic!("Test certificate creation failed - this would need a real certificate for testing")
        })
    }
