...
//!
//! This module provides various progress indicators including progress bars,
//! spinners, and percentage-based progress tracking for file operations.
...
impl TerminalProgress {
    ...
    fn render_percentage(&self, current: u64, total: u64, message: &str) {
        ...
        print!("\r🔄 {} - {}% ({}/{}) - {:.1}s", 
               message, percentage, current, total, elapsed.as_secs_f64());
    ...
    fn render_progress_bar(&self, current: u64, total: u64, message: &str) {
        ...
        print!("\r🔄 {} [{}] {:.1}%{}", 
               message, bar, percentage * 100.0, eta);
        io::stdout().flush().unwrap();
    }
...
impl ProgressReporter for TerminalProgress {
    ...
    fn finish(&self) {
        *self.is_finished.lock().unwrap() = true;
        ...
        let elapsed = self.start_time.elapsed();
        let message = self.current_message.lock().unwrap().clone();
        println!("✅ {} - Completed in {:.1}s", message, elapsed.as_secs_f64());
    }
    ...
    fn finish_with_error(&self, error: &str) {
        *self.is_finished.lock().unwrap() = true;
        ...
        let elapsed = self.start_time.elapsed();
        let message = self.current_message.lock().unwrap().clone();
        println!("❌ {} - Failed after {:.1}s: {}", message, elapsed.as_secs_f64(), error);
    }
}
