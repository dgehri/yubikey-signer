...
use crate::types::{PivSlot, TimestampUrl};
...
impl ConfigManager {
    ...
    pub fn load_or_create_default(&self) -> SigningResult<SigningConfiguration> {
        if self.config_path.exists() {
            ...
            log::info!("Configuration file not found, creating default: {}", self.config_path.display());
    ...
    pub fn load(&self) -> SigningResult<SigningConfiguration> {
        ...
        let content = fs::read_to_string(&self.config_path)
            .map_err(|e| SigningError::ConfigurationError(format!(
                "Failed to read config file {}: {}", self.config_path.display(), e
            )))?;
    ...
    pub fn save(&self, config: &SigningConfiguration) -> SigningResult<()> {
        ...
        if let Some(parent) = self.config_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| SigningError::ConfigurationError(format!(
                    "Failed to create config directory {}: {}", parent.display(), e
                )))?;
        }
        ...
        fs::write(&self.config_path, content)
            .map_err(|e| SigningError::ConfigurationError(format!(
                "Failed to write config file {}: {}", self.config_path.display(), e
            )))?;
    ...
    fn validate_config(&self, config: &SigningConfiguration) -> SigningResult<()> {
        ...
        config.default_hash_algorithm.parse::<HashAlgorithm>()
            .map_err(|_| SigningError::ConfigurationError(
                format!("Invalid hash algorithm: {}", config.default_hash_algorithm)
            ))?;
    ...
    pub fn update_value(&self, key: &str, value: &str) -> SigningResult<()> {
        ...
        match key {
            "default_piv_slot" => {
                let slot_value: u8 = value.parse()
                    .map_err(|_| SigningError::ConfigurationError(
                        format!("Invalid PIV slot value: {value}")
                    ))?;
            ...
            "default_hash_algorithm" => {
                value.parse::<HashAlgorithm>()
                    .map_err(|_| SigningError::ConfigurationError(
                        format!("Invalid hash algorithm: {value}")
                    ))?;
            ...
            "embed_certificate" => {
                config.embed_certificate = value.parse()
                    .map_err(|_| SigningError::ConfigurationError(
    ...
    pub fn import_config(&self, content: &str, format: ExportFormat) -> SigningResult<()> {
        let config: SigningConfiguration = match format {
            ExportFormat::Toml => toml::from_str(content)
                .map_err(|e| SigningError::ConfigurationError(format!("TOML import failed: {e}")))?,
            ExportFormat::Json => serde_json::from_str(content)
                .map_err(|e| SigningError::ConfigurationError(format!("JSON import failed: {e}")))?,
            ExportFormat::Yaml => serde_yaml::from_str(content)
                .map_err(|e| SigningError::ConfigurationError(format!("YAML import failed: {e}")))?,
        };
...
impl ProfileManager {
    ...
    pub fn list_profiles(&self) -> SigningResult<Vec<String>> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| SigningError::ConfigurationError(format!(
    ...
    pub fn save_profile(&self, profile: &ConfigProfile) -> SigningResult<()> {
        ...
        fs::write(&profile_path, content)
            .map_err(|e| SigningError::ConfigurationError(format!(
                "Failed to write profile {}: {}", profile_path.display(), e
            )))?;
    ...
    pub fn load_profile(&self, name: &str) -> SigningResult<ConfigProfile> {
        ...
        let content = fs::read_to_string(&profile_path)
            .map_err(|e| SigningError::ConfigurationError(format!(
                "Failed to read profile {}: {}", profile_path.display(), e
            )))?;
...
mod tests {
    ...
    fn test_config_serialization() {
        ...
        assert_eq!(config.primary_timestamp_server, deserialized.primary_timestamp_server);
    }
