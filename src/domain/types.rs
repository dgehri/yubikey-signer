//! Type-safe wrappers using new-type pattern for domain objects.
//!
    ...
    fn validate_url(url: &str) -> SigningResult<()> {
        ...
        for pattern in &suspicious_patterns {
            if url.contains(pattern) {
                return Err(SigningError::ValidationError(format!(
                    "Timestamp URL contains suspicious pattern '{pattern}': {url}"
                )));
            }
        }
...
impl PivPin {
    ...
    fn validate_pin(pin: &str) -> SigningResult<()> {
        ...
        if pin.len() < 6 {
            return Err(SigningError::ValidationError(format!(
                "PIV PIN too short: {} characters (minimum 6)",
                pin.len()
            )));
        }
        ...
        if pin.len() > 8 {
            return Err(SigningError::ValidationError(format!(
                "PIV PIN too long: {} characters (maximum 8)",
                pin.len()
            )));
        }
...
/// NOTE: Consider converting to enum with defined variants for better type safety
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct PivSlot(u8);

impl PivSlot {
    /// Authentication slot (0x9a) - typically used for login/authentication certificates
    pub const AUTHENTICATION: PivSlot = PivSlot(0x9a);

    /// Digital signature slot (0x9c) - typically used for code signing certificates
    pub const SIGNATURE: PivSlot = PivSlot(0x9c);

    /// Key management slot (0x9d)
    pub const KEY_MANAGEMENT: PivSlot = PivSlot(0x9d);

    /// Card authentication slot (0x9e)
    pub const CARD_AUTHENTICATION: PivSlot = PivSlot(0x9e);
...
impl Default for PivSlot {
    fn default() -> Self {
        PivSlot::AUTHENTICATION // Default to Authentication slot
    }
}
...
impl FromStr for PivSlot {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let slot = u8::from_str_radix(s, 16).map_err(|_| {
            SigningError::ValidationError(format!(
                "Invalid slot format '{s}'. Expected hex value (9a, 9c, 9d, or 9e)"
            ))
        })?;
        Self::new(slot)
    }
...
impl HashData {
    ...
    pub fn raw(data: Vec<u8>) -> SigningResult<Self> {
        if data.len() > 1024 {
            return Err(SigningError::ValidationError(format!(
                "Hash data too large: {} bytes (maximum 1024)",
                data.len()
            )));
        }
        Ok(HashData {
    ...
    fn validate_path(path: &std::path::Path) -> SigningResult<()> {
        ...
        for pattern in &suspicious_patterns {
            if path_str.contains(pattern) {
                return Err(SigningError::ValidationError(format!(
                    "Path contains suspicious pattern '{pattern}': {path_str}"
                )));
            }
        }
        ...
        if path_str.len() > 1000 {
            return Err(SigningError::ValidationError(format!(
                "Path too long: {} characters (maximum 1000)",
                path_str.len()
            )));
        }
...
mod tests {
    ...
    fn test_piv_slot_validation() {
        ...
        for slot in valid_slots {
            assert!(
                PivSlot::new(slot).is_ok(),
                "Slot should be valid: 0x{slot:02x}"
            );
        }
        ...
        for slot in invalid_slots {
            assert!(
                PivSlot::new(slot).is_err(),
                "Slot should be invalid: 0x{slot:02x}"
            );
        }
    }
