...
impl MsiFile {
    ...
    pub fn embed_signature_with_ex(
        ...
        // Remove existing signature streams if present
        if cfb.exists(super::DIGITAL_SIGNATURE_STREAM) {
            cfb.remove_stream(super::DIGITAL_SIGNATURE_STREAM)
                .map_err(|e| {
                    SigningError::MsiParsingError(format!(
                        "Failed to remove existing signature: {e}"
                    ))
                })?;
        }
        if cfb.exists(super::DIGITAL_SIGNATURE_EX_STREAM) {
            cfb.remove_stream(super::DIGITAL_SIGNATURE_EX_STREAM)
                .map_err(|e| {
                    SigningError::MsiParsingError(format!(
                        "Failed to remove existing extended signature: {e}"
                    ))
                })?;
        }

        // Write the main signature stream
        let mut sig_stream = cfb
            .create_stream(super::DIGITAL_SIGNATURE_STREAM)
            .map_err(|e| {
                SigningError::MsiParsingError(format!("Failed to create signature stream: {e}"))
            })?;
        sig_stream.write_all(pkcs7_der).map_err(|e| {
            SigningError::MsiParsingError(format!("Failed to write signature: {e}"))
        })?;

        // Write extended signature if provided
        if let Some(ex_data) = msi_digital_signature_ex {
            let mut ex_stream = cfb
                .create_stream(super::DIGITAL_SIGNATURE_EX_STREAM)
                .map_err(|e| {
                    SigningError::MsiParsingError(format!(
                        "Failed to create extended signature stream: {e}"
                    ))
                })?;
            ex_stream.write_all(ex_data).map_err(|e| {
                SigningError::MsiParsingError(format!("Failed to write extended signature: {e}"))
            })?;
        }

        // Serialize the modified CFB back to bytes
        let mut output = Vec::new();
        cfb.into_inner().set_position(0);

        // We need to re-open and serialize properly
        let cursor = Cursor::new(self.data.clone());
        let mut cfb = CompoundFile::open(cursor)
            .map_err(|e| SigningError::MsiParsingError(format!("Failed to reparse MSI: {e}")))?;

        // Remove and recreate streams
        if cfb.exists(super::DIGITAL_SIGNATURE_STREAM) {
            cfb.remove_stream(super::DIGITAL_SIGNATURE_STREAM)
                .map_err(|e| {
                    SigningError::MsiParsingError(format!("Failed to remove signature stream: {e}"))
                })?;
        }
        if cfb.exists(super::DIGITAL_SIGNATURE_EX_STREAM) {
            cfb.remove_stream(super::DIGITAL_SIGNATURE_EX_STREAM)
                .map_err(|e| {
                    SigningError::MsiParsingError(format!(
                        "Failed to remove extended signature stream: {e}"
                    ))
                })?;
        }
        ...
        {
            let mut sig_stream =
                cfb.create_stream(super::DIGITAL_SIGNATURE_STREAM)
                    .map_err(|e| {
                        SigningError::MsiParsingError(format!(
                            "Failed to create signature stream: {e}"
                        ))
                    })?;
            sig_stream.write_all(pkcs7_der).map_err(|e| {
                SigningError::MsiParsingError(format!("Failed to write signature: {e}"))
        ...
        // Create extended signature stream if needed
        if let Some(ex_data) = msi_digital_signature_ex {
            let mut ex_stream = cfb
                .create_stream(super::DIGITAL_SIGNATURE_EX_STREAM)
                .map_err(|e| {
        ...
        let mut inner = cfb.into_inner();
        inner.set_position(0);
        inner.read_to_end(&mut output).map_err(|e| {
            SigningError::MsiParsingError(format!("Failed to serialize signed MSI: {e}"))
        })?;
