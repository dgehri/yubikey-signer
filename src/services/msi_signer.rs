...
impl MsiSigner {
    ...
    pub fn compute_tbs_hash_with_context(&self, msi_data: &[u8]) -> SigningResult<MsiTbsContext> {
        ...
        log::debug!(
            "Computed MSI hash: {} bytes, extended: {}",
            msi_hash.len(),
            msi_ex_hash.is_some()
        );
        ...
        log::debug!(
            "Computed TBS hash for MSI: {} bytes (MSI hash was {} bytes)",
            tbs_hash.len(),
            msi_hash.len()
        );
    ...
    fn create_spc_content(&self, msi_hash: &[u8]) -> SigningResult<Vec<u8>> {
        // MSI uses SPC_SIPINFO_OBJID for the data blob type
        // The structure is similar to PE but with MSI-specific UUID
        let mut content = Vec::new();

        // Build SpcAttributeTypeAndOptionalValue for MSI
        let mut spc_attribute = Vec::new();

        // OID for SPC_PE_IMAGE_DATA_OBJID (we use the same for MSI hash)
        ...
        // SpcPeImageData structure
        let mut spc_pe_image_data = vec![0x03, 0x02, 0x07, 0x80]; // BIT STRING with flags

        // SpcString (file link) - "<<<Obsolete>>>" in UTF-16LE
        let spc_string = vec![
        ...
        spc_pe_data_complete.extend(self.encode_length(spc_pe_image_data.len()));
